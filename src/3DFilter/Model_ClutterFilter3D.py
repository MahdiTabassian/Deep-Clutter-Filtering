...
def attention_gate_block_3D(x, g, n_inter_filters=None, name=None, **prm):
    ...
    Parameters
    ----------
    x: an encoding output tensor, i.e. set of generated feature maps, at level i
    g: a decoding output tensor at level i+1 (in the encoding direction)
    n_inter_filters: number of the filters generated after merging the x and g filters
...
def conv_block(x, filters_list, act='linear', kernel_size=3, stride=1, pad='same', drp=0.05, name=None):
    ...
    Parameters
    ----------
    x: input tensor which is also used as the new generated inputs/outputs in the function
...
def Unet3D(x_in, name, **config):
    """
    Spatiotemporal clustter filtering based on 3D Unet
...
def clutter_filter_3D(**config):
    """
    Main function for designing the clutter filtering algorithm
